...
export function RoomColumn({ status, rooms }: RoomColumnProps) {
  ...
  return (
    <GlassPanel 
      className={cn(
        "flex flex-col h-full min-h-[500px] md:min-h-0 p-5 transition-all duration-300 relative overflow-hidden",
    ...
    >
      ...
      <div className="flex flex-col gap-4 flex-1 overflow-y-auto hide-scrollbar pb-6 z-10">
        {rooms.map((room) => (
      ...
      <div 
        ref={setNodeRef}
        className={cn(
          "mt-4 p-6 rounded-[20px] border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center text-center gap-3 shrink-0",
          isOver 
